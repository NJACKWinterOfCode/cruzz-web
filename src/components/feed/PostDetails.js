...
class PostDetails extends Component {
  ...
  componentDidMount() {
    this.props.loading();
    axios
      .get("https://cruzz.herokuapp.com/api/post/view/" + this.props.match.params.slug + "/")
      .then(res => {
        this.setState({
          posts: [res.data.post]
        });
  ...
  deleteComment(event, pk) {
    const delURI =
      "https://cruzz.herokuapp.com/api/post/" + this.props.match.params.slug + "/comments/delete/" + pk + "/";
    axios
      .get(delURI)
      .then(res => {
        this.getComments();
  ...
  editComment(event, pk) {
    const edtURI =
      "https://cruzz.herokuapp.com/api/post/" + this.props.match.params.slug + "/comments/update/" + pk + "/";
    let comment = {
      comment: {
  ...
  getComments() {
    this.props.loading();
    const URI =
      "https://cruzz.herokuapp.com/api/post/" + this.props.match.params.slug + "/comments/view?limit=100&offset=0";
    axios
      .get(URI)
      .then(res => {
        this.setState({
  ...
  //need to be changed here hopefully
  handleDateTime(date) {
    var today = new Date();
  ...
  render() {
    return (
      <div
        className="uk-container uk-align-center uk-margin-remove-bottom"
        data-uk-scrollspy="cls: uk-animation-slide-bottom-medium; target: > div; delay: 40;"
      >
        {this.state.posts.length > 0 ? (
                ...
                <div key={key} data-uk-scrollspy="cls: uk-animation-slide-bottom-medium; target: > div; delay: 40;">
                  ...
                  <div className="uk-card uk-card-secondary uk-box-shadow-large uk-align-center uk-width-4-5@m">
                    <form className="uk-padding-small" onSubmit={this.handleComment.bind(this)}>
                      ...
                      />
                      <button
                        className="uk-button uk-button-default uk-margin-small-top uk-margin-small-bottom uk-align-center uk-width-2-5"
                        type="submit"
                      >
                        comment
                      </button>
          ...
          <div>
            <img alt="loading" className="uk-align-center" src={spinner} />
          </div>
        )}
                        ...
                        <div className="uk-grid-small uk-width-1-1 uk-flex-inline" datauk-grid="true">
                          ...
                          <div className="uk-width-1-1">
                            <Link
                              ...
                              to={
                                comment.author.username === this.props.auth.user.username
                                  ? "/profile/" + this.props.auth.user.username
                            ...
                            {comment.author.username === this.state.posts[0].author.username ? (
                              <span
                                className="uk-margin-small-left uk-align-right"
                                uk-icon="star"
                                data-uk-tooltip="title: author; pos: top"
                              />
                            ) : null}
                            {comment.author.official_page ? (
                              <span
                                className="uk-margin-small-left uk-align-right uk-animation-scale-down"
                                uk-icon="check"
                                data-uk-tooltip="title: official; pos: top"
                              />
                            ) : null}
                            <p className="uk-text-meta uk-margin-remove-top">
                              <time dateTime="2016-04-01T19:00">
                                {" "}
                                commented on {this.handleDateTime(comment.createdAt)}
                              </time>
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="uk-card-footer uk-width-1-1 uk-flex-inline">
                        {this.props.auth.user.username === comment.author.username ? (
                          <div className="uk-animation-scale-down">
                            <button
                              data-uk-toggle={"target: #edt-cmnt-" + comment.id}
                              ...
                              <div className="uk-modal-dialog uk-modal-body">
                                <form className="uk-padding-small" onSubmit={e => this.editComment(e, comment.id)}>
                                  <input
                                    ...
                                    placeholder="type your comment..."
                                  />
                                  <p>hit enter to update your comment or click cancel to go back</p>
                                  <div className="uk-flex-inline uk-width-1-1">
                                    <button
                                      className="uk-button uk-button-secondary uk-margin-small-top uk-modal-close"
                                      type="button"
                                    >
                                      Cancel
                                    </button>
                                    {this.props.auth.loading ? (
                                      <div
                                        className="uk-text-right uk-align-right uk-margin-small-right uk-animation-scale-up"
                                        data-uk-spinner="ratio: 2"
                                      />
                                    ) : (
                                      <span
                                        className="uk-margin-small-right uk-align-right uk-animation-scale-down uk-text-success uk-animation-reverse"
                                        data-uk-icon="icon: check; ratio: 2"
                                      />
                                    )}
                                  </div>
                                </form>
                        ...
                        {this.props.auth.user.username === comment.author.username ? (
                          <div className="uk-animation-scale-down">
                            <Link
                              ...
                              className="uk-icon-button uk-text-danger uk-margin-small-left uk-button-secondary"
